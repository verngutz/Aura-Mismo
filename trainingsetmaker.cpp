...
int main() {
   ...
   // populate training files with stft data from each training wav files
   for( int i = 0; i < NUM_WAVES; i++ ) {
		...
		ifstream label_files[NUM_KEYS];
		int labels[NUM_KEYS];
		for( int j = 0; j < NUM_KEYS; j++ ) {
			stringstream ss;
   		ss << LABEL_FILE << i << "_" << j << ".label";
			label_files[j].open( ss.str().c_str() );
			label_files[j] >> labels[j];
			label_files[j].close();
		}
	...
	while( chunk_pos < signal_length && !stop ) {
		...
		for( int i = 0; i < NUM_KEYS; i++ ) {
			...
			for( int j = 0; j < window_size; j++ ) {
				training_files[i] << " " << (j+1) << ":" << (fft_result[j][0] * fft_result[j][0]);
